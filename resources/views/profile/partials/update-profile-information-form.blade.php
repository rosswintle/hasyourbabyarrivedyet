<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-dashboard.text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $user->name)" required autofocus autocomplete="name" />
            <x-dashboard.input-error class="mt-2" :messages="$errors->get('name')" />
        </div>

        <div>
            <x-dashboard.input-label for="email" :value="__('Email')" />
            <x-dashboard.text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $user->email)" required autocomplete="username" />
